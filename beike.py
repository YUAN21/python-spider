...
def first():
    ...
    try:
        response = requests.get(
            'https://zj.ke.com/ershoufang/dantuqu/', headers=headers, timeout=7)
        if response.status_code == 200:
            ...
            for li in sell_list:
                ...
                soup_detail = BeautifulSoup(response_detail.text, 'html5lib')
                ...
                community_area_info_dom = community_dom.find(
                    'div', class_="areaName").find('span', class_="info")   # 小区所在区
                community_zone = community_area_info_dom.contents[0].string   # 小区所在商圈
                ...
                house_info_dom = soup_detail.find('div', class_="houseInfo")
                ...
                if build_year_re:
                    build_year = build_year_re.groups(1)[0]
                ...
                for li in base_intro:
                    ...
                    if label == '房屋户型':
                        ...
                        floor = re.match(re_floor_position, content).groups()
                        ...
                        print('       卍卍卍卍卍卍卍卍 【基本信息】没有写入的属性：' + label + ' 卐卐卐卐卐卐卐卐')
                ...
                for li in transaction_intro:
                    ...
                    if content == '暂无数据':
                        continue
                    if label == '挂牌时间':
                        listing_time_re = re.match(re_listing_time, content).groups()
                        listing_time = listing_time_re[0] + '-' + listing_time_re[1] + '-' + listing_time_re[2]
                    elif label == '交易权属':
                        ...
                        print('       卍卍卍卍卍卍卍卍 【交易信息】 没有写入的属性：' + label + ' 卐卐卐卐卐卐卐卐')

        else:
            print(response.status_code)
    except RequestException:
        print('发生错误' + RequestException)
